        ...
        ast.mapIndexed { index, node ->
            if (node is Function) {
                ...
                node.parameters.mapIndexed { index2, parameter ->
                    ...
                    assertEquals(expectedData[index].parameterType?.get(index2), parameter.type.typeString)
                }

                assertEquals(expectedData[index].returnType, node.returnType?.typeString)
            }
        }
    }
    ...
    fun parserVarTest() {
        ...
        ast.mapIndexed { index, astNode ->
            if (astNode is Statement.VariableDeclaration) {
                ...
                assertEquals(expectedData[index].type, astNode.type?.typeString)
    ...
    fun parserPathTest() {
        ...
        ast.mapIndexed { index, astNode ->
            ...
            astNode.expression.mapIndexed { index2, expression ->
                ...
                } else if (expression is Expression.VariableValueExpression) {
    ...
    fun parserReturnStatementTest() {
        ...
        ast.mapIndexed { index, astNode ->
            ...
            when (val expression = astNode.expression) {
                ...
                is Expression.VariableValueExpression -> assertEquals(expectedData[index], expression.value.literal)
